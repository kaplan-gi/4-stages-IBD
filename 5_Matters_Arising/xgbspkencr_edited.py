"""
Script Name:        xgbspkencr_edited.py
Description:        Calculates top-5 feature imporantance lists using XGBoost, Spearman's correlation, and Kendall's tau
...
if 'status' in data.columns:
    data = data[data['status'] != 'unlabelled'] # LH: LINE EDITED TO REMOVE ALL UNLABELLED DATA # data = data.drop('status', axis=1)
...
# LH: ADDED LINES 159–180 TO CREATE OUTPUT CSV FOR EASIER COPY AND PASTE OF RESULTS
...
df_list = [
    make_long_df(xgb_imp_full[top5_xgb_full], 'XGB_full'),
    make_long_df(xgb_imp_reduced[top5_xgb_reduced], 'XGB_reduced'),
...
combined_df.to_csv('XGB_feature_importances.csv', index=False)
